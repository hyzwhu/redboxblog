...
1. 以上是一个load之后的block片段，但是这个block只存在于形而不存在于神，需要使用reduce方法来执行这个block中的代码，使其成为真正的键对值形式的block。（其实此时你可以从上面的编码中隐隐约约看到地图的轮廓）
...
以上draw-map方法，主要思路是通过读取从第一步骤中得到的地图信息中具体的关卡信息存在level-data中，再初始化一块背景图片，使用for-pair方法
...
以上代码y坐标+16主要是因为之前设想到要给button预留位置，但是之后发现16的高度不足以良好的显示一个button，所以之后改变了这个大小。
...
我们可以将box-world中的mad-man改变为以上形式。rate用于控制小人两个动作之间交换的频率（当你设置的越高小人原地“动的”越快）。
